...
export class FitbitSubscriptionService {
  ...
  async createSubscription(
    ...
    try {
      ...
      // Create subscription via FitBit API
      ...
      if (!response.ok && response.status !== 409) {
        // 409 means the sub already exists on FitBit's end
        const errorText = await response.text();
        logger.error(`Failed to create FitBit subscription: ${response.status} ${errorText}`);
        return null;
      }

      // Parse response (works for both 201 Created and 409 Conflict)
      ...
      // We may already have a sub if the use is relinking, so update or create
  ...
  async deleteSubscription(
    ...
    try {
      ...
      // Mark as inactive and delete from database
   ...
   * Useful for keeping local state in sync
   */
  async syncSubscriptions(account: ProviderAccount): Promise<void> {
    try {
      ...
      // Mark subscriptions that no longer exist in FitBit as inactive
